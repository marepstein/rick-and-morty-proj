...
const IntroPage: NextPage = () => {
  ...
  const [clickedCharacter, setClickedCharacter] = useState<LickApi.ICharacter>();
  const [isLoading, setIsLoading] = useState<boolean>(false);

  const router = useRouter();

  const getSingleCharacterID = async (e: LickApi.ICharacter) => {
    try {
  ...
  const getData = async () => {
    setIsLoading(true);
    const { data: characters } = await axios.get<LickApi.ICharacter[]>('/api/characters');
